...
def upload_Csv(file_name, bucket, object_name=None):
    ...
    try:
        response = session.upload_Csv(file_name, bucket, object_name)
        print("CSV para o S3 com sucesso")
        ...
        if len(servidor) == 0:
            ...
            with open(arquivo_csv, 'a', newline='') as csvfile:
                ...
                """)
                    ...
                    upload_Csv(arquivo_csv, bucket_name, nome_arquivo_s3)
                    time.sleep(5)    
                    return("ok")
