...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Lê o CSV tratado diretamente do S3
...
def dashAlertasGestora(dados, geral, bucket):
    ...
    for (empresa, datacenter, zona, servidor), (data_leitura, linha) in ultimas_leituras.items():
        ...
        if not empresa or not datacenter or not zona or not servidor:
            print(f"⚠️ [DADOS INCOMPLETOS] Pulando linha do CSV. Valores encontrados -> Empresa: '{empresa}', DC: '{datacenter}', Zona: '{zona}', Servidor: '{servidor}'")
            continue
        # ---------------------------------------------------------

        try:
            # Usando a função auxiliar para buscar ignorando Case Sensitive
            ...
            print(f"[AVISO] Servidor {servidor} não encontrado no metricas.json ({str(e)}) — pulando.")
            continue
    ...
    for linha in dados:
        chave = (
            str(linha.get("EMPRESA",    "")).strip(),
            str(linha.get("DATACENTER", "")).strip(),
            str(linha.get("ZONA",       "")).strip(),
            str(linha.get("SERVIDOR",   "")).strip()
        )
        data_str = str(linha.get("DATE", ""))
        try:
            data_linha = datetime.fromisoformat(data_str).replace(tzinfo=None)
